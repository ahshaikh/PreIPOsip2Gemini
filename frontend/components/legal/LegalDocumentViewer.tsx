...
export default function LegalDocumentViewer({
  ...
  const shouldFetch = !customContent;
  ...
  const { data: acceptanceStatus } = useQuery({
    ...
    enabled: showAcceptance && requireAuth && shouldFetch,
  });

  const acceptMutation = useMutation({
    mutationFn: () => api.post(`/legal/documents/${documentType}/accept`),
    onSuccess: () => {
      toast.success("Document Accepted", { description: "Your acceptance has been recorded." });
      setHasAccepted(true);
      queryClient.invalidateQueries({ queryKey: ['legalAcceptance', documentType] });
    },
    onError: (e: any) => {
      toast.error("Error", { description: e.response?.data?.message || "Failed to record acceptance" });
    }
  });
  ...
  const downloadPDF = async () => {
    if (!document) return toast.error("PDF unavailable");
    try {
  ...
  // STATIC MODE (customContent enabled)
  if (customContent) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-background to-muted/20 py-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Review the following policy carefully.</p>
          </div>
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-muted-foreground">{document.description || `Read our ${title} carefully`}</p>
            </div>
