...
export function ReturnsCalculator() {
  ...
  // Simple estimation logic based on the "10% Bonus" rule
  const totalInvested = monthly * months;
  const baseBonus = totalInvested * 0.10; // 10% base
  const referralBonus = baseBonus * (referrals * 0.1); // Extra 10% per referral (simplified)
      ...
      <Card className="bg-primary text-primary-foreground">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-between text-lg">
            ...
            <span className="font-bold text-green-300">+₹{(baseBonus + referralBonus).toLocaleString()}</span>
          </div>
