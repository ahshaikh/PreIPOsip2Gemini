 ...
 *
 * Admin Actions List - Navigation to individual admin action audits
 *
 * INVARIANTS:
 * - READ-ONLY list view
...
import {
  listAuditAdminActions,
  type AuditListItem,
  type PaginatedResponse,
} from "@/lib/auditApi";
import { AuditTimestamp } from "@/components/audit";

export default function AuditActionsListPage() {
  ...
  useEffect(() => {
    async function loadData() {
      setLoading(true);
      try {
        const result = await listAuditAdminActions({ page, per_page: 20 });
        setData(result);
      } catch (err) {
        console.error("[Audit] Failed to load actions:", err);
        setError("Failed to load admin actions list.");
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm font-mono text-slate-500 dark:text-slate-400 mt-1">
          Complete log of all platform admin actions. Select an action to view
          full details including actor, reason, and state changes.
        </p>
      </div>
              ...
              {actions.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link
                      href={`/admin/audit/actions/${item.id}`}
