...
class VerifyOtpAction
{
    ...
    public function execute(int $userId, string $type, string $code): array
    {
        ...
        if (!$user) {
            // Security: Don't reveal user existence explicitly
            throw ValidationException::withMessages(['otp' => 'Invalid request details.']);
        }
        ...
        // If both email and mobile are now verified, set status to ACTIVE
        $activated = false;
        $user->refresh();
